...
- `src/lib/ai/` model calls (`provider`), per-user credentials (`aiCredentials`), capability gating + model catalogs (`capabilities`, merging the old registry/groqCaps/googleCaps), coach turn orchestration (`coach`, prompt text in `coachPrompt`, context/snapshot building in `coachContext`, the write-approval flow in `coachApproval`), tools (`coachTools` + catalog search/ranking in `coachCatalogSearch`, `writeGate` for the write blocklist, with the capability-derived gate itself in `capabilities`), spend/abuse constants (`limits`: turn cap, tool-step cap, continuation-retry cap), memory (`memory` summary, `facts` + `embeddings`), background maintenance (`maintenance`: daily stale-fact expiry + memory consolidation via `/api/cron/maintenance`), ingestion (`vision`, `inbody`, `mdExtract`, `mdImport`)
